...
// ── Selector de opciones con botones ───────────────────────────────
...
export default function SizeAdjustment() {
  ...
  // Cargar tallas al montar
  useEffect(() => {
    if (!user) { navigate("/login", { replace: true }); return; }
    const fetchSizes = async () => {
      try {
        const token = getCookie("auth_token");
        const data = await sizeService.getSizes(token);
        if (data) reset(data);
      } catch {
        // Sin datos aún → form vacío para que complete
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const token = getCookie("auth_token");
      const saved = await sizeService.updateSizes(token, data);
      reset(saved);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-lg rounded-2xl shadow-xl overflow-hidden bg-white text-black">
        ...
        {/* Header — mismo patrón que Profile */}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 flex flex-col gap-6">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-gray-500">
              Guarda tus medidas para que podamos recomendarte outfits perfectos.
            </p>
          </div>
          ...
          <SizeSelector
            ...
            onChange={(v) => setValue("clothingSize", v, { shouldDirty: true })}
            error={errors.clothingSize?.message}
          />

          {/* Talla de zapatos */}
          <SizeSelector
            label="Talla de zapatos (CL)"
            options={SHOE_SIZES_CL}
            value={watchedShoe}
            onChange={(v) => setValue("shoeSize", v, { shouldDirty: true })}
            error={errors.shoeSize?.message}
          />

          {/* Talla de jeans */}
          <SizeSelector
            label="Talla de jeans (cintura)"
            options={JEAN_SIZES}
            value={watchedJean}
            onChange={(v) => setValue("jeanSize", v, { shouldDirty: true })}
            error={errors.jeanSize?.message}
          />
          ...
          <SizeSelector
            ...
            options={FIT_TYPES}
            value={watchedFit}
            onChange={(v) => setValue("fitPreference", v, { shouldDirty: true })}
          ...
          <div className="flex flex-col gap-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">
              Medidas corporales (cm/kg) — opcionales
            </p>
