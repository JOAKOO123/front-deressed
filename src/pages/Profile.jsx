...
const profileSchema = z.object({
  ...
  city: z.string().max(60, "Máximo 60 caracteres").optional(),
  country: z.string().max(60, "Máximo 60 caracteres").optional(),
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {props.as === "textarea" ? (
        <textarea
          rows={3}
          className={`border rounded-lg px-4 py-2.5 text-sm outline-none transition-colors resize-none
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        // Si no hay perfil aún, el form queda vacío para que complete
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const token = getCookie("auth_token");
      const saved = await profileService.updateProfile(token, data);

      // Actualizar nombre en el contexto/sidebar
      updateUserProfile({
        name: `${saved.firstName} ${saved.lastName}`,
      });

      reset(saved); // marca el form como "no sucio" de nuevo
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 flex flex-col gap-5">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-gray-500">
              {user?.email} — completa tu perfil para una mejor experiencia.
            </p>
          </div>

          {/* Avatar placeholder */}
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center text-2xl text-gray-400 shrink-0">
              👤
            </div>
          ...
          <Field
            as="textarea"
            label="Sobre mí"
            placeholder="Cuéntanos algo sobre tu estilo o preferencias..."
